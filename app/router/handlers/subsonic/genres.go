...
// parse/guard/store body is shared with updateAlbum and updateArtist via
// updateManualCover.
func (h *Handler) updateGenre(w http.ResponseWriter, r *http.Request) {
	h.updateManualCover(w, r, "updateGenre", "genre", assetstore.KindGenre, func(id uint) (string, []string, error) {
		genre, err := h.store.GetGenre(id)
		if err != nil {
			return "", nil, err
		}
		return assetkey.GenreOf(genre), nil, nil
	})
}
