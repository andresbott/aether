...
//
// Every /api/v1 failure answers {"error": "<sentence>", "code": "<slug>"}, and
// the backend now writes user-facing sentences for upstream problems (see
// internal/upstream). This module is the single place the UI reads that shape,
// so no view has to re-derive `err?.response?.data?.error ?? err.message` — and
// so a body that is itself JSON is never shown raw, which is exactly what used
// to leak {"error":...,"code":"upstream_error"} onto the screen.

const GENERIC_MESSAGE = 'Something went wrong. Please try again.'
const NETWORK_MESSAGE = 'The server could not be reached. Check your connection and try again.'

/** The error envelope every /api/v1 handler answers with. */
...
function unwrapNested(msg: string): string {
    ...
    for (let depth = 0; depth < 3 && current.startsWith('{'); depth++) {
        try {
            ...
            const inner = typeof parsed.error === 'string' ? parsed.error.trim() : ''
            if (inner === '') return ''
            current = inner
        } catch {
...
export function apiErrorMessage(err: unknown, fallback: string = GENERIC_MESSAGE): string {
    ...
    if (typeof data === 'object' && data !== null) {
        ...
        if (typeof body.error === 'string' && body.error.trim() !== '') {
            const unwrapped = unwrapNested(body.error)
            if (unwrapped !== '') return unwrapped
        }
    }
...
export function isRateLimitError(err: unknown): boolean {
    ...
    const data = responseData(err)
    if (typeof data === 'object' && data !== null) {
        return (data as ApiErrorBody).code === 'upstream_rate_limited'
    }
    return false
}
