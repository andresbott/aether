...
describe('HeroActions', () => {
    ...
    it('renders a filled heart when canStar and starred, and emits star', async () => {
        ...
        expect(star.find('.msf-favorite').exists()).toBe(true)
    ...
    it('shows the outline heart when not starred', () => {
        ...
        expect(w.find('.hero-action-star .ms-favorite').exists()).toBe(true)
        expect(w.find('.hero-action-star .msf-favorite').exists()).toBe(false)
    })
