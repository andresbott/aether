...
describe('scanTokens', () => {
    it('finds ms- and msf- class tokens', () => {
        const src = `<i class="ms-play-arrow"></i> icon: 'msf-favorite' :class="x ? 'ms-pause' : ''"`
        expect([...scanTokens(src, 'a.vue')].sort()).toEqual(['ms-pause', 'ms-play-arrow', 'msf-favorite'])
    })
    ...
    it('rejects a dynamically built icon class', () => {
        ...
        expect(() => scanTokens("const c = 'msf-' + name", 'a.ts')).toThrow(/dynamic/)
    })
})

describe('buildIconCss', () => {
    const css = buildIconCss(set, ['ms-play-arrow', 'msf-favorite'])
    it('gives every icon class and .app-icon the shared mask rule', () => {
        ...
        expect(common).toContain('.msf-favorite')
