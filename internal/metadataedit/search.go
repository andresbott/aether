...
// relative to the scan folder and forward-slash, exactly as ListFolders reports
// them, so a result round-trips back through the folder API. Results are
// sorted by path.
...
// confined to the scan folder root — the same confinement ListFolders relies on.
