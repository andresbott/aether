...
- [x] logout should stop playing, empty the local storage and clear tanstack cache — `purgeLocalSession` in `webui/src/composables/useAuth.ts`: unbinds the queue sync, clears the queue, empties localStorage and resets the query cache. Runs on an expiry too.
- [x] Proxy-header auth mode (Authelia / X-Forwarded-User) — implemented: `Auth.Method: proxy-header` with configurable identity headers, `TrustedProxies` CIDR check, JIT user provisioning into the shared userdb, role live from the groups header, token mint/CRUD via the injected `Caller` seam, SPA reload-on-401. See `docs/agents/authentication.md` (mode: proxy-header) and `app/router/proxy_auth.go`.
...
- [ ] Multi-user — per-user scoping of queue, stars, playlists, and history landed via session identity (owner-keyed schemas). PAT layer landed (replaces the interim cookie resolver and unblocks third-party clients). Remaining work: per-user schema niceties (user ID instead of login string, cleaner renames). Known caveat: **owner is the LOGIN string**, so renaming a user detaches their owner-keyed data (queue, stars, playlists, history); acceptable pre-1.0, revisit later.
