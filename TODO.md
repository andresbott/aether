...
- [/] Expose Release type as an editable field
  **Emit shipped** (branch `feat/opensubsonic-release-types`): the album type is now read as the full multi-value list (`tags.Metadata.ReleaseTypes`, taglib + ffprobe via the shared `allOrSingle` MusicBrainz splitter), stored as a `[]string` `serializer:json` column (`model.Album.ReleaseTypes`, populated in `scanner/reconcile.go`), and serialized on every album response as the standard OpenSubsonic `AlbumID3` fields `releaseTypes` + derived `isCompilation` (`subsonic/browsing.go` `albumToMap`, so all six album endpoints carry them). Base OpenSubsonic fields, not an extension — nothing to advertise. Remaining: (2) **consume** it — `LibraryView` type filter + `ArtistView` discography grouping; (3) **edit** it — the metadata-editor correction control (single-select primary + multi-select secondary). Full handoff in `docs/local/release-type-editable-field.md`.
