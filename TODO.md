...
- [ ] Authentication — implement real user auth with Subsonic token validation. The model is **decided**: see [`docs/agents/authentication.md`](docs/agents/authentication.md) (two modes, `builtin` and `proxy-header`, sharing one token layer). Implement it, don't invent alternatives
...
- [ ] Subsonic external-client auth — third-party Subsonic apps (Symfonium, DSub, etc.) authenticate with the Subsonic protocol's own query-string credentials. Implement via per-user **Personal Access Tokens (PATs)**, added as a generic CRUD feature in the `userauth` library (aether wires routing, authorization, and the management UI).
    ...
    - Wiring: a thin Subsonic `AuthHandler` parses `u`/`t`/`s`/`p`/`apiKey` and validates against the PAT verifier. Per `authentication.md`, **`/rest` is token-only in every mode** — the SPA mints itself a token on boot and speaks standard Subsonic auth, so this is load-bearing for aether's own UI, not just for foreign clients.
...
## Frontend — Finish favorites/starring

**Every toggle now exists.** Album detail, artist view, song detail, player
now-playing, grid cards, playlists (via the `playlistStar` extension) and — as of
the track-row work — every track row: `AlbumTrackRow`, `GenreTrackRow` (genre
detail, search, playlist detail) and `QueueRow` in view mode, all rendering one
shared `TrackFavoriteButton` through one composable, `useSongFavorite`. All use
the `pi pi-heart(-fill)` icon with "Add to/Remove from favorites" wording, and
`/rest` emits `starred` on artists/albums/songs/playlists so state survives a
reload. **What's left is that nothing browses the result**, which is what makes
the feature a dead end for users:

- [x] ~~Star/unstar toggle on track rows (album view and queue)~~ — done, and
      extended to all three row components rather than the two listed, so no track
      row in the app lacks a heart. Two notes for anyone touching this: rows must be
      handed the queue's *actual* song object (`QueueBody` passes `{ song, queueIndex }`,
      not a spread copy) because `useSongFavorite` writes `starred` locally — the queue
      is not query-backed, so nothing else would refresh the heart; and `QueueRow`'s
      view-mode row is now a `role="button"` div with hand-wired Enter/Space, because
      a heart nested inside a real `<button>` is invalid HTML
- [ ] Starred library section — browse starred albums, artists, tracks **and
      playlists** (backed by `getStarred2`)
      (FULLY OPEN, and now the *only* thing blocking this feature. This used to be partially
      answered by the Discovery "Favorites" section at `/discover/favorites`, but that route was
      deleted when Discovery became a single ranked feed. Favorites are now only a scoring term in
      that feed — a ranking boost, not visible or browsable (the reason badge that used to surface
      it was removed too). **Nothing browses starred items today: no albums, no artists, no
      tracks, no playlists** — so a user can star things and never find them again.
      The server side is done and untouched by the toggle work: `getStarred2` is registered,
      library-scopable via `musicFolderId`, and already returns starred playlists with counts and
      duration — but it appears **nowhere** in `webui/`: no client method, no type, no query key,
      no view. Two things to know before building it: `store.GetStarred`'s album/artist/track
      queries have **no `ORDER BY`** (only playlists order by star recency), so "recently
      favorited" needs a store change or a client-side sort on the `starred` timestamp each entity
      already carries; and `getStarred2` is unpaginated by spec, so don't design the view around an
      offset param. Open design call: standalone `/favorites` route vs. a fourth `LibraryView` tab —
      standalone is preferable, since the playlist array is deliberately *not* library-scoped
      (`store/star.go:120-122`) while the rest is, which reads as mixed semantics inside a
      folder-scoped tab.)
...
- [ ] Favorites schema — three junction tables (`album_stars`, `artist_stars`, `track_stars`), each with composite PK `(user_id, item_id)`, a `starred_at` timestamp, and cascade deletes on user/item removal; replace the current single `starred_items` table if one exists. Blocked on multi-user — the whole point of the rework is the `user_id` half of the key
