...
- [ ] Path traversal validation in `stream` and `getCoverArt` — validate that resolved file paths are within configured music directories before serving. Both now go through `serveETaggedFile` → `http.ServeContent` (`subsonic/media.go:391`), and neither takes a path from the request: `stream` resolves it from the track ID via `GetTrackFilePath` (`media.go:32`), covers come from the cover resolver. So this is defense in depth against a DB row holding a path outside any library (metadata editor, stale rows) rather than a live traversal hole — the `//nolint:gosec` at `media.go:395` asserts exactly that assumption, and nothing enforces it. `metadataedit/paths.go:25` already has the containment helper shape to reuse.
...
- [ ] `albumToMap` missing `songCount`/`duration` when tracks are not preloaded — confirmed: `albumToMap` only emits them under `len(al.Tracks) > 0` (`subsonic/browsing.go:234-242`), and `GetAlbumList` preloads just `Artists`/`Genres` (`store/album.go:90`) while the detail query preloads `Tracks` (`album.go:69`). So every list caller — `lists.go:44,139`, `browsing.go:115`, `search.go:60`, `discovery.go:123` — omits both fields and clients render "0 songs". Prefer a batched count/duration aggregate over preloading whole tracks into list responses.

## Backend — Data Integrity & Scanning

- [ ] Use `errors.Is(err, gorm.ErrRecordNotFound)` in `FindOrCreateArtists` and `FindOrCreateAlbum` to distinguish not-found from real DB errors — today a genuine DB failure during a scan is treated as "not found". Still bare `if err != nil { create }` at `store/artist.go:21` and `store/album.go:16`; neither file imports `errors` yet. `store/playqueue.go:30,66,150` is the pattern to copy.
...
- [ ] Album cover on the library grid can show another album's image after metadata edits + rescan (works correctly on the album detail view). Remaining root causes:
    - Embedded-cover lookup `GetCoverTrackPath` returns the *first* track with `has_embedded_cover=true`, with no ordering — which track wins is unstable across rescans. Fix: order deterministically (e.g. lowest `(disc, track)`) — the query at `internal/store/track.go:130-135` still has no `Order`.
...
- [ ] `getPlaylists` N+1 queries — each playlist triggers separate count and duration queries; consider a single annotated query (still present: `GetPlaylistTrackCount`/`GetPlaylistDuration` per row at `playlists.go:103-104`). Stars and play stats are already batched alongside it (`StarredAt`, `PlaylistStats`) — follow that shape.
