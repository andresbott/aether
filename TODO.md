...
- [ ] Review the 25-tokens-per-user default (pat.Opts.MaxPerUser): with a 48h spa expiry and mint-time-only sweep, ~25 fresh boots in 48h hit the cap (409 on mint). Decide a policy (raise the cap, sweep more aggressively, or reuse live spa tokens).
