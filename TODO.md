...
- [/] Expose Release type as an editable field
  **Emit + manual edit shipped** (branch `feat/opensubsonic-release-types`, verify-green). Emit: read as a multi-value list (`tags.Metadata.ReleaseTypes`), `[]string serializer:json` album column, serialized as OpenSubsonic `releaseTypes` + derived `isCompilation` on all six album endpoints. Edit: the metadata editor reads/edits release types end to end — `metadataedit.Patch.ReleaseTypes` → `MUSICBRAINZ_ALBUMTYPE` (managed tag), `release_types` on the Track DTO + patch, and a primary single-select + secondary multi-select `EditPanel` control over the fixed MusicBrainz vocabulary (flat storage, UX-only split; the iTunes Compilation checkbox stays separate). Remaining: (a) **identify-pull** — stage a matched release group's primary+secondary types from MusicBrainz on an identify pick (rides the `useReleaseGroupGenres` rail); (b) **consume** — `LibraryView` type filter + `ArtistView` discography grouping. Design in `docs/local/release-type-editor-design.md`.
