...
func TestStyleRenders(t *testing.T) {
	...
	data, err := g.GenerateStyle("smoke", 64, lowpoly.Style)
	if err != nil {
		t.Fatal(err)
	}
	...
	if lowpoly.Style.Name() != "lowpoly" {
		t.Fatalf("Name = %q", lowpoly.Style.Name())
	}
}
...
func TestStyleUsesInjectedPalette(t *testing.T) {
	...
	if bytes.Equal(renderWith(t, lowpoly.New(stubPalette{red})), renderWith(t, lowpoly.New(stubPalette{blue}))) {
