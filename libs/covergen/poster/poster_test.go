...
func TestStyleRenders(t *testing.T) {
	g := covergen.New(poster.Style)
	data, err := g.GenerateStyle("smoke", 64, poster.Style)
	if err != nil {
		t.Fatal(err)
	}
	...
	if poster.Style.Name() != "poster" {
		t.Fatalf("Name = %q", poster.Style.Name())
	}
}
...
func TestStyleUsesInjectedPalette(t *testing.T) {
	...
	if bytes.Equal(renderWith(t, poster.New(stubPalette{red})), renderWith(t, poster.New(stubPalette{blue}))) {
