...
func TestStyleRenders(t *testing.T) {
	...
	data, err := g.GenerateStyle("smoke", 64, halftone.Style)
	if err != nil {
		t.Fatal(err)
	}
	...
	if halftone.Style.Name() != "halftone" {
		t.Fatalf("Name = %q", halftone.Style.Name())
	}
}
...
func TestStyleUsesInjectedPalette(t *testing.T) {
	...
	if bytes.Equal(renderWith(t, halftone.New(stubPalette{red})), renderWith(t, halftone.New(stubPalette{blue}))) {
