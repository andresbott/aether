...
func TestStyleRenders(t *testing.T) {
	g := covergen.New(remix.Style)
	data, err := g.GenerateStyle("smoke", 64, remix.Style)
	if err != nil {
		t.Fatal(err)
	}
	...
	if remix.Style.Name() != "remix" {
		t.Fatalf("Name = %q", remix.Style.Name())
	}
}
...
func TestStyleUsesInjectedPalette(t *testing.T) {
	...
	if bytes.Equal(renderWith(t, remix.New(stubPalette{red})), renderWith(t, remix.New(stubPalette{blue}))) {
