// Package classic implements the covergen "classic" style: a two-colour
// diagonal gradient with translucent white shapes.
package classic
...
// Style is the classic cover-art style with the default palette.
...
// shipped look).
var classicKnobs = []covergen.Knob{
...
func drawForeground(img *image.RGBA, rng *rand.Rand, ks covergen.KnobSet) {
	...
	randAlpha := func() uint8 {
		return paint.ClampU8(int(math.Round(paint.ClampFloat(paint.SampleAround(rng, ocenter, owidth), 0, 1) * 255)))
	}
...
func drawShape(img *image.RGBA, kind shapeKind, cx, cy, radius int, maxAlpha uint8, gradientAngle float64) {
	...
	for y := y0; y < y1; y++ {
		for x := x0; x < x1; x++ {
			...
			paint.BlendPixel(img, x, y, color.RGBA{R: 255, G: 255, B: 255, A: a})
		}
	}
}
