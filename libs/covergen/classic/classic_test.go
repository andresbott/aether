...
func TestStyleRenders(t *testing.T) {
	g := covergen.New(classic.Style)
	data, err := g.GenerateStyle("smoke", 64, classic.Style)
	if err != nil {
		t.Fatal(err)
	}
	...
	if classic.Style.Name() != "classic" {
		t.Fatalf("Name = %q", classic.Style.Name())
	}
}
...
func TestStyleUsesInjectedPalette(t *testing.T) {
	...
	if bytes.Equal(renderWith(t, classic.New(stubPalette{red})), renderWith(t, classic.New(stubPalette{blue}))) {
