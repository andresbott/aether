...
func TestStyleRenders(t *testing.T) {
	...
	data, err := g.GenerateStyle("smoke", 64, liquid.Style)
	if err != nil {
		t.Fatal(err)
	}
	...
	if liquid.Style.Name() != "liquid" {
		t.Fatalf("Name = %q", liquid.Style.Name())
	}
}
...
func TestStyleUsesInjectedPalette(t *testing.T) {
	...
	if bytes.Equal(renderWith(t, liquid.New(stubPalette{red})), renderWith(t, liquid.New(stubPalette{blue}))) {
