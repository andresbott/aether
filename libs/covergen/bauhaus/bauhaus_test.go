...
func TestStyleRenders(t *testing.T) {
	g := covergen.New(bauhaus.Style)
	data, err := g.GenerateStyle("smoke", 64, bauhaus.Style)
	if err != nil {
		t.Fatal(err)
	}
	...
	if bauhaus.Style.Name() != "bauhaus" {
		t.Fatalf("Name = %q", bauhaus.Style.Name())
	}
}
...
func TestStyleUsesInjectedPalette(t *testing.T) {
	...
	if bytes.Equal(renderWith(t, bauhaus.New(stubPalette{red})), renderWith(t, bauhaus.New(stubPalette{blue}))) {
