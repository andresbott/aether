...
func TestStyleRenders(t *testing.T) {
	...
	data, err := g.GenerateStyle("smoke", 64, rings.Style)
	if err != nil {
		t.Fatal(err)
	}
	...
	if rings.Style.Name() != "rings" {
		t.Fatalf("Name = %q", rings.Style.Name())
	}
}
...
func TestStyleUsesInjectedPalette(t *testing.T) {
	...
	if bytes.Equal(renderWith(t, rings.New(stubPalette{red})), renderWith(t, rings.New(stubPalette{blue}))) {
