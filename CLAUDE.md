...
| `LibraryView` | `/library/:mode?`, `/library/:folderId/:mode?` | Browse the catalog — whole, or one library at a time (`:folderId` is the library's id, Subsonic's `musicFolderId`; not a scan folder). Modes — Discover / Releases / Artists — are switched from the sidebar's Library block and are real path segments (bare `/library` is Discover, then `/library/releases` and `/library/artists`; within a folder, `/library/:folderId/releases`), not a URL hash or an in-view selector. Discover is a ranked feed of albums + playlists (`getDiscovery` extension, infinite scroll, `DiscoveryFeed` component) and is the default mode on the root `/library` only — the ranking is cross-library, so discovery is never library-scoped; there is deliberately no standalone `/discover` route — the mode segment is constrained to `releases`/`artists`, so `/library/discover` is not addressable and Discover is only ever the bare `/library` root. Releases carries a release-type tab row (All + MusicBrainz primary types: Album/Single/EP/Broadcast/Other) via the `releaseTypeFilter` extension, hidden while the favorites filter is on |
