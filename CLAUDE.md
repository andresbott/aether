...
| `LibraryView` | `/library/:folderId?` | Browse the music library by folder. Modes — Discover / Releases / Artists / Songs — are switched from the sidebar's Library block (`#discover`/`#releases`/`#artists`/`#songs` hash), not an in-view selector. Discover is a ranked feed of albums + playlists (`getDiscovery` extension, infinite scroll, `DiscoveryFeed` component) and is the default mode on the root `/library` only — the ranking is cross-collection, so discovery is never library-scoped; there is deliberately no standalone `/discover` route. Releases carries a release-type tab row (All + MusicBrainz primary types: Album/Single/EP/Broadcast/Other) via the `releaseTypeFilter` extension, hidden while the favorites filter is on |
