...
| `AlbumView` | `/album/:id` | Single album detail. The hero eyebrow names its release types (`lib/releaseTypes.ts` `formatReleaseTypes`: primary first, e.g. "Album · Soundtrack"), "Album" when it has none. Hero cover is editable (`updateAlbum` / `albumCoverArt` extension) — the only place a manual album cover is set, since the metadata editor is file-only |
